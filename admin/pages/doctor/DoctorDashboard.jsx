...
const DoctorDashboard = () => {
  ...
  const getStatus = (item) => {
    if (item.cancelled) return { label: 'Cancelled', className: 'bg-red-50 text-red-700 border-red-200' };
    if (item.isCompleted) return { label: 'Completed', className: 'bg-emerald-50 text-emerald-700 border-emerald-200' };
    if (checkTreatment(item, 30)) return { label: 'Live Now', className: 'bg-emerald-50 text-emerald-700 border-emerald-200 animate-pulse' };
    return { label: 'Scheduled', className: 'bg-gray-100 text-gray-600 border-gray-200' };
  };
        ...
        <div className="bg-white border-2 border-black p-6 hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all duration-200">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-black mt-2">{dashboardData.appointment || 0}</p>
            </div>
            <div className="p-3 bg-gray-100 rounded-lg">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        <div className="lg:col-span-2 bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-bold tracking-tight">Latest Appointments</h2>
            <span className="text-xs font-semibold text-gray-400">{dashboardData.latestAppointments?.length || 0} today</span>
          </div>

          <div className="hidden md:grid grid-cols-[2.2fr_0.8fr_1.1fr_0.8fr_0.9fr_1fr] gap-3 px-2 pb-3 mb-1 border-b border-gray-100 text-[11px] font-bold uppercase tracking-wider text-gray-400">
            <span>Patient</span>
          ...
          <div className="divide-y divide-gray-100">
            {dashboardData.latestAppointments?.map((item, index) => {
              ...
              return (
                <div
                  key={item._id || index}
                  className="grid grid-cols-1 md:grid-cols-[2.2fr_0.8fr_1.1fr_0.8fr_0.9fr_1fr] gap-3 items-center py-4 px-2 first:pt-0 last:pb-0"
                >
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="font-bold text-sm tracking-tight text-black truncate">{user.name || 'N/A'}</p>
                      <p className="text-xs text-gray-500 truncate">{user.email || 'N/A'}</p>
                      <p className="text-xs text-gray-400 truncate">{user.phone || 'N/A'}</p>
                    </div>
                  </div>

                  <div className="text-sm font-semibold text-gray-700 md:text-center">
                    {calculateAge(user.dob)} <span className="text-gray-400 font-normal">&middot; {user.gender || 'N/A'}</span>
                  </div>

                  <div className="text-sm">
                    <p className="font-semibold text-black">{item.slotTime || 'N/A'}</p>
                    <p className="text-xs text-gray-400">{item.slot_date || 'N/A'}</p>
                  </div>
                  ...
                  <div className="md:flex md:justify-end">
                    <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold border ${status.className}`}>
                      {status.label === 'Live Now' && <span className="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>}
                      {status.label === 'Live Now' ? 'LIVE NOW' : status.label}
                    </span>
                  </div>
                </div>
              );
